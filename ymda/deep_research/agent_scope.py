...
from ymda.deep_research.prompts import transform_messages_into_research_topic_human_msg_prompt, draft_report_generation_prompt, clarify_with_user_instructions
from ymda.deep_research.state_scope import AgentState, ResearchQuestion, AgentInputState, DraftReport
...
def get_today_str() -> str:
    ...
    # Remove leading zero from day if present (e.g., "Dec 05" -> "Dec 5")
    return date_str.replace(" 0", " ")

# ===== CONFIGURATION =====

# Initialize model lazily to avoid requiring API key at import time  
def _get_models():
    from langchain.chat_models import init_chat_model
    return (
        init_chat_model(model="openai:gpt-4.1-mini"),
        init_chat_model(model="openai:gpt-4.1-mini")
    )
...
def write_research_brief(state: AgentState) -> Command[Literal["write_draft_report"]]:
    ...
    stats.extract_usage_from_response(
        response,
        model_name="model",
        function_name="write_research_brief"
    )
...
def write_draft_report(state: AgentState) -> Command[Literal["__end__"]]:
    ...
    stats.extract_usage_from_response(
        ...
        function_name="write_draft_report"
    )
