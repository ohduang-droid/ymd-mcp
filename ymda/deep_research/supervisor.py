...
from ymda.deep_research.prompts import lead_researcher_with_multiple_steps_diffusion_double_check_prompt
from ymda.deep_research.researcher import researcher_agent
from ymda.deep_research.state_multi_agent_supervisor import (
    SupervisorState, 
    ConductResearch,
    ResearchComplete
)
from ymda.deep_research.utils import get_today_str, think_tool, refine_draft_report
from ymda.deep_research.token_stats import get_token_stats

def validate_and_fix_messages(messages: list[BaseMessage]) -> list[BaseMessage]:
    """验证并修复消息历史，确保每个带有 tool_calls 的 assistant 消息都有对应的 tool 响应.
    
    从前往后检查消息，如果发现带有 tool_calls 的 AIMessage 后面没有对应的 ToolMessage，
    则移除该 AIMessage 及其之后的所有消息。
    ...
    while i < len(messages):
        ...
        if isinstance(msg, AIMessage) and hasattr(msg, 'tool_calls') and msg.tool_calls:
            ...
            # 如果没有有效的 tool_call id，直接保留消息（可能是格式问题，但不影响）
            ...
            # 检查所有 tool_calls 是否都有对应的响应
            tool_response_ids = {tm.tool_call_id for tm in found_tool_messages if hasattr(tm, 'tool_call_id') and tm.tool_call_id}
            
            # 如果所有 tool_calls 都有响应，保留这对消息
            if tool_call_ids.issubset(tool_response_ids):
                valid_messages.append(msg)
                valid_messages.extend(found_tool_messages)
                # 跳过已处理的 ToolMessage
                i = j
            else:
                # 如果工具调用不完整，停止处理，不包含这个 AIMessage
                break
        else:
            # 其他类型的消息直接保留
            valid_messages.append(msg)
...
async def supervisor(state: SupervisorState) -> Command[Literal["supervisor_tools"]]:
    ...
    # 验证并修复消息历史，确保消息完整性
    validated_messages = validate_and_fix_messages(supervisor_messages)
    ...
    messages = [SystemMessage(content=system_message)] + validated_messages
    ...
    stats.extract_usage_from_response(
        response,
        model_name="supervisor_model_with_tools",
        function_name="supervisor"
    )
